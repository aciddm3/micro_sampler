// изменённый  ParamSlider nih_plug_egui
use std::sync::{Arc, LazyLock};

use nih_plug::prelude::{Param, ParamSetter};
use nih_plug_egui::egui::emath::GuiRounding;
use nih_plug_egui::egui::{
    self, Color32, Key, Response, Sense, Stroke, TextEdit, TextStyle, Ui, Vec2, Widget, WidgetText,
    emath, pos2, vec2,
};
...
impl<'a, P: Param> ParamSlider<'a, P> {
    pub fn for_param(param: &'a P, setter: &'a ParamSetter<'a>, color: Color32) -> Self {
        Self {
            param,
            setter,
            color,
    ...
    fn slider_ui(&self, ui: &Ui, response: &mut Response) {
        ...
        if ui.is_rect_visible(response.rect) {
            ...
            if filled_proportion > 0.0 {
                let triangle_center = response.rect.left_center()
                    + vec2(response.rect.width() * filled_proportion, 0.0);
                let triangle_length = response.rect.height() / 2.0;
                let p1 = triangle_center + vec2(triangle_length, triangle_length);
                let p2 = triangle_center + vec2(-triangle_length, triangle_length);
                let p3 = triangle_center + vec2(0.0, -triangle_length);
                ui.painter()
                    .line_segment([p1, p2], Stroke::new(1.0, self.color));
                ui.painter()
                    .line_segment([p2, p3], Stroke::new(1.0, self.color));
                ui.painter()
                    .line_segment([p3, p1], Stroke::new(1.0, self.color));
            }
